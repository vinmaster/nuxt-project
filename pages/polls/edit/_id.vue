...
export default {
  ...
  async asyncData(context) {
    let response = await context.$axios.get(`/api/polls/${context.params.id}`);
    const poll = response.data.poll;
    if (!poll) return { poll: {} };
    const options = poll.options.map(o => o.option);
    return { poll, form: {
      title: poll.title,
      category: poll.category,
      visibility: poll.visibility,
      adding: poll.addingBy !== null,
      autoclose: poll.closeAt !== null,
      closeDateTime: poll.closeAt,
      options,
    } };
  },
  data() {
    ...
    async onSubmit() {
      ...
      this.$refs.form.validate(async (valid) => {
        ...
        try {
          ...
          const response = await this.$axios.patch('/api/polls', {
            ...this.form,
          });
